export const HOME = '/'
export const USER_PROFILE = '/profile'
export const CONTACT = '/kontakt'
export const ABOUT_US = '/o-nas'
export const FAQ = '/faq'
export const TERMS_AND_CONDITIONS = '/regulamin'
export const PRIVACY_POLICY = '/polityka-prywatnosci'

export const ADMIN_PANEL = '/panel-administratora'

export const REGISTER_COMAPNY = '/rejestracja-firmy'
export const COMPANY_PANEL = '/panel-firmy'
export const COMPANY_EMPLOYEE_VIEW = COMPANY_PANEL + '/pracownik/:id'

export const EMPLOYEE_PANEL = '/panel-pracownika'
export const EMPLOYEE_DOCUMENTS = EMPLOYEE_PANEL + '/dokumenty'
export const EMPLOYEE_CALENDAR = EMPLOYEE_PANEL + '/kalendarz'
export const EMPLOYEE_TASK = EMPLOYEE_PANEL + '/zadanie/:id'
export const EMPLOYEE_TOOL = EMPLOYEE_PANEL + '/narzedzie/:id'
